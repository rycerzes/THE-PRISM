...
import { Collection, Message,  MessageActionRow,  MessageAttachment } from "discord.js";
    ...
    async messageRun(message: Message) {
        ...
        while (!end) {
            ...
            const promise = await Promise.race([
                ...
                sent.awaitMessageComponent({ time: 600 * 1000, filter: (interaction) => interaction.user.id === message.author.id })
            ]);

            if (promise.constructor.name === 'ButtonInteraction') {
            ...
            guessMessage = (promise as Collection<string, Message<boolean>>).first();
            guess = guessMessage?.content.toUpperCase();
